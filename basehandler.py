...
class BaseHandler(webapp2.RequestHandler):
    ...
    def send_email(self):
        try:
            ...
            mail.send_mail(sender=from_addr,
                            to='mengfeng0904@gmail.com',
                            subject=subject,
                            body=message)
            json_string='{"status":"ok", \
                        "message":"send email to Z successfully."}'
            ...
            json_string='{"status":"fail", \
                        "message":"cannot send email to Z."}'
...
class Location(BaseHandler):
    ...
    def location(self):
        ...
        if dispatcher=='get_by_html5':
            ...
            self.get_locations();
        elif dispatcher=='get_request_stats':
            self.get_request_stats()

    def get_request_stats(self):
        try:
            #retrieve the distinct country list
            json_dict={}
            country_list=db.GqlQuery("SELECT DISTINCT location_country FROM RequestorLocation").run(limit=300)
            json_dict['request_stats']={}
            json_dict['status']='ok'
            json_dict['message']='Gets request stats successfully'
            for country in country_list:
                if country.location_country=='': pass
                else:
                    location_country_results=RequestorLocation.all().filter("location_country =",country.location_country).fetch(2000)
                    request_count=sum([ location_country_result.requests_count for location_country_result in location_country_results]) 
                    json_dict['request_stats'][country.location_country]=request_count
    ...
    def get_locations(self):
        try:
            ...
            results=RequestorLocation.all().order('-updated').fetch(2000)
