...
class BaseThoughtHandler(BaseHandler):
    @property
    def data_class(self):
        return ZThoughts
    def get_records(self):
        ...
        template=jinja_environment.get_template('templates/zthought.html')
        template_values={
                    'page_title':'ZThought',
    ...
    def insert_new_record(self,data_class):
        try:
            ...
            content=self.request.get('description','No description')
...
class ZThought(BaseThoughtHandler):
    def get(self):
...
class Plist(webapp2.RequestHandler):
    def get(self):
        ...
        plist = plistlib.writePlistToString(getplist())
